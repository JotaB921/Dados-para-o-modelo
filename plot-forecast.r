...
make_forecast_plot <- function(){
  ...
  load(paste0("~/Videos/covid19model-master/results/", filename))
  all_data <- data.frame()
  ...
  for(i in 1:length(countries)){
    ...
    uti_li <- colQuantiles(uti[,1:N,i], probs=.025)
    uti_ui <- colQuantiles(uti[,1:N,i], probs=.975)
...
make_single_plot <- function(data_country, data_country_forecast, filename, country){
  ...
  ggsave(file= paste0("~/Videos/covid19model-master/figures/", country, "_forecast_", filename, ".png"), 
         p, width = 10)
  
  # Produce plots for Website
  dir.create("~/Videos/covid19model-master/web/figures/desktop/", showWarnings = FALSE, recursive = TRUE)
  save_plot(filename = paste0("~/Videos/covid19model-master/web/figures/desktop/", country, "_forecast", ".svg"), 
  ...
  save_plot(filename = paste0("~/Videos/covid19model-master/web/figures/mobile/", country, "_forecast", ".svg"), 
